...
void VulkanRenderer::setLoadFactor(int factor) {
    loadFactor = factor;
}
...
void VulkanRenderer::createInstance() {
    ...
    const char* extensionNames[] = { VK_KHR_SURFACE_EXTENSION_NAME, VK_KHR_ANDROID_SURFACE_EXTENSION_NAME };
...
void VulkanRenderer::createSwapchain(int width, int height) {
    ...
    createInfo.imageFormat = VK_FORMAT_R8G8B8A8_UNORM; // Fallback to a common format
...
void VulkanRenderer::createGraphicsPipeline() {
    const uint32_t app_src_main_shaders_triangle_vert[] =
#include "triangle_vert.h"
    ;
    const uint32_t app_src_main_shaders_triangle_frag[] =
#include "triangle_frag.h"
    ;
    VkShaderModule vertShaderModule = createShaderModule(device, app_src_main_shaders_triangle_vert, sizeof(app_src_main_shaders_triangle_vert));
    VkShaderModule fragShaderModule = createShaderModule(device, app_src_main_shaders_triangle_frag, sizeof(app_src_main_shaders_triangle_frag));
...
void VulkanRenderer::render() {
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        // We'll let resize handle this usually, but a robust app would recreate here.
        return;
    ...
    vkCmdPushConstants(commandBuffers[imageIndex], pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(int), &loadFactor);
